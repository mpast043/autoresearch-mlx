...
def _sorted_recurrence_match_records(records: Any) -> list[dict[str, Any]]:
...
def _reviewable_recurrence_matches_by_source(evidence: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    ...
    for label in labels:
        strong_records = _sorted_recurrence_match_records(strong_by_source.get(label, []))
        partial_records = _sorted_recurrence_match_records(partial_by_source.get(label, []))
        if strong_records or partial_records:
            reviewable[label] = [*strong_records, *partial_records]
    return reviewable


def _best_surfaced_evidence(row: dict[str, Any]) -> dict[str, Any]:
...
def _confidence_posture(row: dict[str, Any], brief_payload: dict[str, Any]) -> str:
...
def _repeatability_posture(row: dict[str, Any]) -> str:
...
def _next_recommended_action(row: dict[str, Any], build_brief_present: bool) -> str:
...
class Database:
    ...
    def get_recent_validations(self, limit: int = 25) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            matched_docs_by_source = {
                source: _sorted_recurrence_match_records(records)
            ...
            results.append(
                {
                    **dict(row),
                    ...
                    "reviewable_recurrence_matches_by_source": _reviewable_recurrence_matches_by_source(
                        {
                            "matched_docs_by_source": matched_docs_by_source,
                            "partial_docs_by_source": partial_docs_by_source,
                        }
    ...
    def get_validation_review(self, limit: int = 25, *, run_id: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            if corroboration:
                corr_evidence = _json_loads(corroboration["evidence_json"], {})
                result["recurrence_state"] = corroboration["recurrence_state"]
                result["corroboration_score"] = corroboration["corroboration_score"] or 0.0
                result.update({
            ...
            if market:
                market_evidence = _json_loads(market["evidence_json"], {})
                result.update({
                    "demand_score": market["demand_score"] or 0.0,
                    "buyer_intent_score": market["buyer_intent_score"] or 0.0,
                    "competition_score": market["competition_score"] or 0.0,
                    "trend_score": market["trend_score"] or 0.0,
                    "review_signal_score": market["review_signal_score"] or 0.0,
                    "value_signal_score": market["value_signal_score"] or 0.0,
                    "wedge_name": market_evidence.get("wedge_name", ""),
                    "wedge_active": market_evidence.get("wedge_active", False),
                    "wedge_fit_score": market_evidence.get("wedge_fit_score", 0.0),
                })
    ...
    def get_candidate_workbench(self, limit: int = 25, *, run_id: Optional[str] = None) -> list[dict[str, Any]]:
        ...
        for row in rows:
            ...
            item = {
                ...
                "confidence_posture": _confidence_posture(row, brief_payload),
                "repeatability_posture": _repeatability_posture(row),
                "family_confirmation_count": int(row.get("family_confirmation_count", 0) or 0),
                "best_surfaced_evidence": _best_surfaced_evidence(row),
                ...
                "next_recommended_action": _next_recommended_action(row, build_brief_present),
            }
            items.append(item)
